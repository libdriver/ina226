...
INA226은 I2C™ 또는 SMBUS 호환 인터페이스가 있는 전류 션트 및 전력 모니터입니다. 이 장치는 션트 전압 강하와 버스 공급 전압을 모두 모니터링합니다. 프로그래밍 가능한 교정 값, 변환 시간 및 평균화는 내부 승수와 결합되어 전류(암페어)와 전력(와트)을 직접 판독할 수 있습니다. INA226은 공급 전압과 관계없이 0V~36V까지 변할 수 있는 공통 모드 버스 전압의 전류를 감지합니다. 이 장치는 단일 2.7V~5.5V 공급에서 작동하여 일반적으로 330μA의 공급 전류를 소모합니다. 이 장치는 -40°C~125°C의 작동 온도 범위에서 지정되며 I2C 호환 인터페이스에서 최대 16개의 프로그래밍 가능한 주소를 제공합니다.
